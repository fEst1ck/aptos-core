...
use supra_framework::{extended_checks, path_in_crate, BuildOptions};
use supra_gas_schedule::{MiscGasParameters, NativeGasParameters, LATEST_GAS_FEATURE_VERSION};
use supra_types::on_chain_config::{
    supra_test_feature_flags_genesis, Features, TimedFeaturesBuilder,
};
use supra_vm::natives;
...
fn run_tests_for_pkg(path_to_pkg: impl Into<String>, use_latest_language: bool) {
    ...
    let ok = run_move_unit_tests(
        ...
        utc,
        supra_test_natives(),
        supra_test_feature_flags_genesis(),
...
/// TODO: per @vgao1996:
/// - There should be only one ground truth of `supra_test_natives`.
...
/// More discussion: https://github.com/supra-labs/supra-core/pull/15997#discussion_r1994469668
pub fn supra_test_natives() -> NativeFunctionTable {
...
fn move_token_unit_tests() {
    run_tests_for_pkg("supra-token", false);
}

#[test]
fn move_token_objects_unit_tests() {
    run_tests_for_pkg("supra-token-objects", false);
}

#[test]
fn move_experimental_unit_tests() {
    run_tests_for_pkg("supra-experimental", true);
}
