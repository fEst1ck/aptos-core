spec supra_framework::transaction_fee {
    ...
    spec initialize_fee_collection_and_distribution(supra_framework: &signer, burn_percentage: u8) {
        ...
        aborts_if exists<CollectedFeesPerBlock>(supra_addr);

        ensures exists<ValidatorFees>(supra_addr);
        ensures exists<CollectedFeesPerBlock>(supra_addr);
    }
    ...
    spec schema RequiresCollectedFeesPerValueLeqBlockSupraSupply {
    ...
    spec schema ProcessCollectedFeesRequiresAndEnsures {
        ...
        requires exists<CoinInfo<SupraCoin>>(@supra_framework);
        include RequiresCollectedFeesPerValueLeqBlockSupraSupply;
