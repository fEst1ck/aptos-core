spec supra_framework::account {
    ...
    spec initialize(supra_framework: &signer) {
        ...
        aborts_if exists<OriginatingAddress>(supra_addr);
        ensures exists<OriginatingAddress>(supra_addr);
    }
    ...
    /// Limit the new account address is not @vm_reserved / @supra_framework / @supra_toke.
    spec create_account(new_address: address): signer {
        include CreateAccountAbortsIf {addr: new_address};
        aborts_if new_address == @vm_reserved || new_address == @supra_framework || new_address == @supra_token;
