...
Supra Node. It serves the current "state" readable by Move contracts being
executed, as well as a configurable length of the blockchain history to fellow
Supra Nodes and the Rest API. It takes in new data from either the consensus or
...
a. without the help of widely available healthy running Supra Nodes b. to
...
As part of the Supra Node config, these are specific for the storage components.
...
to be changed. Another reason to not overriding them is the developers of Supra
...
Run the backup coordinator which backs up blockchain data continuously off a Supra Node.
...
OPTIONS:
        --backup-service-address <ADDRESS>
            Backup service address. By default a Supra Node runs the backup service serving on tcp
...
It's part of the Supra API functionality to bootstrap a AptosDB with a backup.
When emergency happens and the need to do the somewhat manual bootstrapping is
high, Supra will provide a backup source in the form of a yaml config file. Otherwise
...
OPTIONS:
    ...
    -h, --help
        ...
        --target-db-dir <DB_DIR>
            ...
            in the backup. The data folder can later be used to start an Supra node. e.g. /opt/
            aptos/data/db
