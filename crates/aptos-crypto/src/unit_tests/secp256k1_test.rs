...
fn serialization() {
    ...
    let signature_deserialized = secp256k1::Signature::try_from(&signature_bytes[..]).unwrap();
    assert_eq!(signature, signature_deserialized);

    let private_key_bytes = key_pair.private_key.to_bytes();
    let private_key_deserialized = secp256k1::PrivateKey::try_from(&private_key_bytes[..]).unwrap();
    assert_eq!(key_pair.private_key, private_key_deserialized);

    let public_key_bytes = key_pair.public_key.to_bytes();
    let public_key_deserialized = secp256k1::PublicKey::try_from(&public_key_bytes[..]).unwrap();
...
fn malleability() {
    ...
    secp256k1::Signature::try_from(&high_signature_bytes[..]).unwrap_err();
}
