...
//! This module provides APIs for private keys and public keys used in SecP256K1.
...
/// SecP256K1 private keys are 256-bit.
pub const PRIVATE_KEY_LENGTH: usize = 32;
/// SecP256K1 public keys contain a prefix indicating compression and two 32-byte coordinates.
pub const PUBLIC_KEY_LENGTH: usize = 65;
/// SecP256K1 signatures are 256-bit.
...
impl PrivateKey {
    ...
    fn sign_arbitrary_message(&self, message: &[u8]) -> Signature {
        ...
        let message = bytes_to_message(&message_digest)
            .expect("Consistently hashed to 32-bytes, should never fail.");
        self.sign(&message)
    }
}
...
impl TryFrom<&[u8]> for PrivateKey {
    ...
    fn try_from(bytes: &[u8]) -> std::result::Result<PrivateKey, CryptoMaterialError> {
        match libsecp256k1::SecretKey::parse_slice(bytes) {
            Ok(public_key) => Ok(PrivateKey(public_key)),
...
fn bytes_to_message(message: &[u8]) -> Result<libsecp256k1::Message> {
    if message.len() != MESSAGE_LENGTH {
        Err(anyhow!(
            "Invalid message length for SecP256K1 Message: {}",
            message.len()
        ))
    } else {
        libsecp256k1::Message::parse_slice(message).map_err(|e| anyhow!("{}", e))
    }
}
