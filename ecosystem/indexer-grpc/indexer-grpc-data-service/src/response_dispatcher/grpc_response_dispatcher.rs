            ...
            for storage in self.storages.as_slice() {
                    ...
                    .await
                {
                    ...
                    Ok(StorageReadStatus::NotAvailableYet) => {
                        ...
                        tokio::time::sleep(Duration::from_millis(NOT_AVAILABLE_RETRY_BACKOFF_IN_MS)).await;
                        break;
                    },
    ...
    async fn fetch_with_retries(&mut self) -> anyhow::Result<Vec<TransactionsResponse>, Status> {
        for _ in 0..FETCH_RETRY_COUNT {
            match self.fetch_internal().await {
                ...
                Err(StorageReadError::PermenantError(s, _e)) => Err(Status::internal(
                    format!("Failed to fetch transactions from storages, {:}",
                    s)
                ))?,
            }
        }
...
mod tests {
    ...
    async fn test_finite_stream() {
        ...
        for t in transactions {
            assert_eq!(t.version, current_version);
            current_version += 1;
        }
    }
    ...
    async fn test_infinite_stream_with_client_closure() {
        ...
        match third_peek {
            ...
            _=> assert!(false, "It's not possible at this point."),
        }
