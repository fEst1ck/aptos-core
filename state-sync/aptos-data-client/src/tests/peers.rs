...
async fn bad_peer_is_eventually_banned_internal() {
    ...
    for poll_priority_peers in [true, false] {
        ...
        let good_peer = mock_network.add_peer(poll_priority_peers);
        let bad_peer = mock_network.add_peer(poll_priority_peers);
        assert_eq!(good_peer.network_id(), bad_peer.network_id());
        let network_id = good_peer.network_id();
...
async fn bad_peer_is_eventually_banned_callback() {
    ...
    for poll_priority_peers in [true, false] {
        ...
        // Add a bad peer
        let bad_peer = mock_network.add_peer(poll_priority_peers);
        let network_id = bad_peer.network_id();
